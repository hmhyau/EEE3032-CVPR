...
% DESCRIPTOR_FOLDER = 'E:\Coursework\descriptors';
DESCRIPTOR_FOLDER = '/Volumes/SANDISK/Coursework/descriptors';
% DESCRIPTOR_FOLDER = '/media/hy00185/SANDISK/Coursework/descriptors';
%% and within that folder, another folder to hold the descriptors
%% we are interested in working with
DESCRIPTOR_SUBFOLDER='EOHColour';


%% 1) Load all the descriptors into "ALLFEAT"
%% each row of ALLFEAT is a descriptor (is an image)
% TODO: Set query image to MOST discriminative classes only
% Limitation on dataset: not strictly a visual search dataset
% causes HUGE error margin during evaluation
ALLFEAT=[];
ALLFILES=cell(1,0);
ctr=1;
...
%% Image ID used during evaluation
% queryimg=[415, 97, 502, 536, 5, 138];
% queryimgname=["Planes", "Books", "Cars", "Bikes", "Flowers", "Chairs"];


%% 3) Compute the distance of image to the query
% [FPCA, vct, val] = cvpr_pca(ALLFEAT'); % Uncomment these two lines
% ALLFEAT = FPCA'; % Uncomment for application of PCA
dst=[];
for i=1:NIMG
    candidate=ALLFEAT(i,:);
...
[precision, recall] = cvpr_prcurve(dst, SHOW); % Change SHOW to NIMG for complete PR Curve
truncdst=dst(1:SHOW,:);
outdisplay=[];
